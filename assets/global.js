...
class QuantityInput extends HTMLElement {
		...
		for (event of eventList) {
			this.input.addEventListener(event, function (e) {
				...
				if (
					...
					if ( this.closest('quick-order-list.list-view-order-list') && e.currentTarget.value == 0 ){
	...
	setMinimumDisable() {
		if ( this.closest('quick-order-list.list-view-order-list')){
...
// Close Predictive search modal on click of close button
document.addEventListener('click', function (e) {
  const closeBtn = e.target.closest('.search__container  .modal-close-button');
  if (!closeBtn) return;

  const detailsEl = closeBtn.closest('details');
  if (detailsEl) {
