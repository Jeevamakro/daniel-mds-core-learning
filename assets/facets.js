...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // We'll add/remove a sticky-bottom class based on scroll
  function checkFacetsStickyBottom() {
    const rect = facetsContainer.getBoundingClientRect();
    const windowHeight = window.innerHeight || document.documentElement.clientHeight;

    // If the top of the facets container has been scrolled past (i.e., it is above the viewport)
    // and at least some part of it is still visible or it's shorter than the viewport
    if (rect.top < 0 && rect.bottom > windowHeight) {
      facetsContainer.classList.add('facets--sticky-bottom');
    } else {
      facetsContainer.classList.remove('facets--sticky-bottom');
    }
  }

  window.addEventListener('scroll', checkFacetsStickyBottom);
  window.addEventListener('resize', checkFacetsStickyBottom);
  // Initial check
  checkFacetsStickyBottom();
});
...
/* Mobile filter fixed button */
  ...
  function windowScrollCP(){
    ...
    const element = document.querySelector('.collection-wrapper .facets-container .mobile-facets__wrapper'),
      element_placeholder = document.querySelector('.collection-wrapper .facets-container'),
      compare_el = document.querySelector('.compare-bottom-wrap:not(.hidden):not(.active)');
      
    const top_1 = element.getBoundingClientRect().top,
    top_2 = element_placeholder.getBoundingClientRect().top;
    let off_top = (top_1 + 42), off_top_2 = (top_2 + 42);
    off_top = off_top - header_height;
    off_top_2 = off_top_2 - header_height;
    const has_class = element.classList.contains('filter-fixed');
    
    if( off_top < 9 && !has_class ){
      ...
      if( compare_el ){
        document.body.style.paddingBottom = compare_el.offsetHeight + element.offsetHeight + 'px';
        element.style.bottom = compare_el.offsetHeight+'px';
      }else{
        document.body.style.paddingBottom = element.offsetHeight + 'px';
      }

    }else if( off_top_2 > 10 && has_class ) {
