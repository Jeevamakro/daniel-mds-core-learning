...
// product spec slideout popup group accordion
document.addEventListener("DOMContentLoaded", () => {

	/* specification and document tab JS under media gallery */
	$(".product-popup-modal__content .spec-feat-title").each(function (index, element) {
		$(element).on("click", function () {
			const parent = $(this).parent();
			var curret_active = $(this).closest('.product-spec-feat-wrapper').find('.spec-feat-item.active');

			if(curret_active.length > 0){
				curret_active.removeClass('active');
				curret_active.find('.spec-feat-list').stop().slideUp(300);
			}else{
				$(this).closest('.spec-feat-item').addClass('active');
				$(this).closest('.spec-feat-item').find('.spec-feat-list').stop().slideDown(300);
			}
		
			console.log(curret_active,$(this).closest('.spec-feat-item'))
			if (
				curret_active.length > 0 &&
				!curret_active.is($(this).closest('.spec-feat-item'))
			) {
				console.log('sdfdf');
				$(this).closest('.spec-feat-item').addClass('active');
				$(this).closest('.spec-feat-item').find('.spec-feat-list').stop().slideDown(300);
	...
	/* specification and document tab JS under media gallery end */
